...
def main() -> None:
    ...
    # Import data
    round_df = pd.DataFrame(json.load(open('round.json')))
    channels_data = json.load(open('channels.json'))
    nums = pd.DataFrame(channels_data['Номера'])
    auditory = pd.DataFrame(channels_data['Аудитории'])
    dates = pd.DataFrame(json.load(open('dates.json')))
    prime = pd.DataFrame(json.load(open('prime.json')))
